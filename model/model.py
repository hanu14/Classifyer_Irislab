...
class CSMN(object):
  ...
  def __init__(self, inputs, config, name="CSMN", is_training=True):
    ...
    self.Wf2 = tf.get_variable(
        "Wf2",
        shape=[150, self.category_size],
        initializer=self.w_initializer
    )

    self.bf2 = tf.get_variable(
        "bf2",
        shape=[self.category_size],
        initializer=self.b_initializer
    )
    ...
    def _loop_cond(iterator_, output_words_array_,
                   ...
                   output_mem_A_, output_mem_B_, output_mem_C_, output_words_array_2):
      if self.is_training:
    ...
    def _loop_body(iterator_, output_words_array_,
      ...
      another_output = layers.fully_connected(
              output,
              num_outputs = 600,
              activation_fn = tf.nn.relu,
              weights_initializer = self.w_initializer,
      ...
      another_output2 = layers.fully_connected(
              another_output,
              num_outputs = 300,
              activation_fn = tf.nn.relu,
              weights_initializer = self.w_initializer,
      ...
      another_output3 = layers.fully_connected(
              another_output2,
              num_outputs = 100,
              activation_fn = tf.nn.relu,
              weights_initializer = self.w_initializer,
      ...
      another_output4 = layers.fully_connected(
              another_output3,
              num_outputs = 50,
              activation_fn = tf.nn.relu,
              weights_initializer = self.w_initializer,
      ...
      output_words_array_2 = output_words_array_2.write(iterator_, another_output4)

      return (iterator_ + 1, output_words_array_, output_mem_state_A_,
          output_mem_state_C_, output_mem_A_, output_mem_B_, output_mem_C_, output_words_array_2)
    ...
    output_words_array2 = tf.TensorArray(
        dtype=tf.float32,
        size=0,
        clear_after_read=False,
        dynamic_size=True
    ...
    sequence_outputs2 = loop_outputs[7].stack()

    sequence_outputs2.set_shape(
        [None, self.batch_size, 50]
    )
    ...
    sequence_outputs2 = tf.transpose(sequence_outputs2, perm=[1, 0, 2])
    sequence_outputs2 = tf.slice(sequence_outputs2, [0, 0, 0], [self.batch_size, 3, 50])
    inputFeature = tf.reshape(sequence_outputs2, [self.batch_size, -1])


    #Tsne = tf.reshape(sequence_outputs2, [self.batch_size, -1, 50])
    #another_output3 = tf.reshape(another_output3, [self.batch_size, -1, 150])
    #TSNE = tf.reduce_max(Tsne,axis=1)
    

    final_outputs = tf.matmul(sequence_outputs, self.Wf) + self.bf
    final_outputs2 = tf.matmul(inputFeature, self.Wf2) + self.bf2
    ...
    prob2 = tf.nn.sigmoid(final_outputs2)
    self.prob2 = prob2
    self.argmax2 = tf.argmax(self.prob2,1)
    
    self.AN=answer_new
    self.loss2 = cross_entropy(
        multi_class_labels=[answer_new],logits=[final_outputs2]
    )
    output_words_array.close()
    output_words_array2.close()
