...
flags.DEFINE_integer('category_size', 20,
    """Number of category."""
)
...
def mk_answer(user_id, category_size, cap=None):
  answer_new = np.zeros([FLAGS.batch_size, category_size], dtype=np.float32)

  for i in range(FLAGS.batch_size):
    #print(user_id[i])
    for j in range(len(user_id[i])):
      if user_id[i][j] == str(1):
        answer_new[i][j]+=1.0
...
def read_numpy_format_and_label(filename_queue):
  ...
  batch_filename, batch_context_length, batch_caption_length, \
      batch_context, batch_caption, batch_category = tf.decode_csv(
  ...
  batch_answer_new = tf.py_func(
      mk_answer,
      [batch_category, FLAGS.category_size],
      tf.float32
  )
  ...
  # DFcor=pd.DataFrame()
  # DFcor['file name']= batch_answer_new
  # DFcor.to_excel(str(FLAGS.eval_dir)+"/CORR_"+str(FLAGS.DF_num)+"+++.xlsx", index=False)

  return batch_img_embedding, batch_context_length, \
      batch_caption_length, batch_context_id, batch_caption_id, \
      batch_answer_id, batch_answer_new, batch_context_mask, batch_caption_mask,batch_filename
...
def enqueue(eval_data):
  ...
  for it in range(FLAGS.num_gpus):
    img_embedding, context_length, caption_length, context_id, caption_id, \
        answer_id, answer_new, context_mask, caption_mask, batch_filename = read_numpy_format_and_label( filename_queue)
    ...
    answer_new.set_shape([FLAGS.batch_size,FLAGS.category_size])
    ...
    img_embeddings, context_lengths, caption_lengths, \
        context_ids, caption_ids, answer_ids, answer_news, context_masks, \
        caption_masks = _generate_data_and_label_batch(
            [img_embedding, context_length, caption_length, \
            context_id, caption_id, answer_id, answer_new, context_mask, caption_mask],
  ...
  return num_examples_per_epoch, img_embedding_list, context_length_list, \
      ...
      answer_new_list, context_mask_list, caption_mask_list,name_list
